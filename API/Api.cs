...
namespace Notus.API
{
    ...
    {
        private DateTime LastNtpTime = NVC.DefaultTime;
        ...
        private Mempool ObjMp_MultiSignPool;
        public Mempool Obj_MultiSignPool
        {
            ...
            if (NVG.Settings.GenesisCreated == false)
            {
                ...
                ObjMp_BlockOrderList = new Notus.Mempool(
                    Notus.IO.GetFolderName(
                        NVG.Settings, NVC.StorageFolderName.Common
                    ) + "ordered_block_list");
                ...
                */
                ...
                ObjMp_MultiSignPool = new Mempool(
                    IO.GetFolderName(
        ...
        public string Interpret(NVS.HttpRequestDetails IncomeData, bool ToDistribute)
        {
            ...
            if (incomeFullUrlPath.StartsWith("multi/"))
            {
                ...
                if (incomeFullUrlPath.StartsWith("multi/pool/"))
                {
                    if (IncomeData.UrlList[2].Length == 90)
                    {
                        ...
                        ObjMp_MultiSignPool.Each((multiKeyId, multiTransferList) =>
                        {
                    ...
                    if (IncomeData.UrlList[2].Length == NVC.WalletFullTextLength)
                    {
                ...
                if (IncomeData.UrlList.Length > 1)
                {
                    ...
                    string multiPrefix = NVC.MultiWalletPrefix;
                    string singlePrefix = NVC.SingleWalletPrefix;
            ...
            if (IncomeData.UrlList.Length > 0)
            {
                ...
                if (IncomeData.UrlList.Length > 1)
                {
                    ...
                    {
                        return Notus.API.TX.Process(IncomeData,ToDistribute);
                    }

                    // alınan işlem transferini veriyor
                    if (string.Equals(IncomeData.UrlList[0].ToLower(), "rx"))
                    {
                        if (IncomeData.UrlList[1].Length == NVC.WalletFullTextLength)
                        {
        ...
        private NVClass.BlockData? GetBlockWithRowNo(long BlockRowNo)
        {
        ...
        private string Request_Layer3_StoreFileNew(NVS.HttpRequestDetails IncomeData)
        {
            ...
            using (Mempool ObjMp_FileChunkList =
                new Mempool(
                    IO.GetFolderName(
            ...
            using (Mempool ObjMp_FileStatus =
                new Mempool(
                    IO.GetFolderName(
                        NVG.Settings.Network,
                        NVG.Settings.Layer,
        ...
        private string Request_Layer3_StoreFileUpdate(NVS.HttpRequestDetails IncomeData)
        {
            ...
            try
            {
                tmpChunkData = JsonSerializer.Deserialize<NVS.FileChunkStruct>(Uri.UnescapeDataString(IncomeData.PostParams["data"]));
            }
            catch (Exception err)
            {
            ...
            {
                ObjMp_FileChunkList.AsyncActive = false;
                ObjMp_FileChunkList.Add(tmpChunkIdKey, Uri.EscapeDataString(tmpChunkData.Data), Val_Timeout);
            }
        ...
        private string Request_StoreEncryptedFile_Update(NVS.HttpRequestDetails IncomeData)
        {
            ...
            //Console.WriteLine(JsonSerializer.Serialize(IncomeData.PostParams, NVC.JsonSetting));
            */
            try
            {
                tmpChunkData = JsonSerializer.Deserialize<NVS.FileChunkStruct>(Uri.UnescapeDataString(IncomeData.PostParams["data"]));
        ...
        private string Request_Layer1_StoreFile_New(NVS.HttpRequestDetails IncomeData)
        {
            ...
            //Console.WriteLine(JsonSerializer.Serialize(NVG.Settings.Genesis.Fee, NVC.JsonSetting));
            ...
            Console.WriteLine(JsonSerializer.Serialize(tmpStorageData, NVC.JsonSetting));
        ...
        private string Request_Block(NVS.HttpRequestDetails IncomeData)
        {
            ...
            if (IncomeData.UrlList[1].Length == 90)
            {
                try
                {
                    ...
                    if (prettyJson == true)
                        return JsonSerializer.Serialize(tmpStoredBlock, NVC.JsonSetting);
            ...
            if (long.TryParse(IncomeData.UrlList[1], out long BlockNumber) == false)
                return JsonSerializer.Serialize(false);
            ...
            if (prettyJson == true)
                return JsonSerializer.Serialize(tmpResultBlock, NVC.JsonSetting);
        ...
        private string Request_BlockHash(NVS.HttpRequestDetails IncomeData)
        {
            ...
            bool isNumeric2 = long.TryParse(IncomeData.UrlList[2], out long BlockNumber2);
            if (isNumeric2 == false)
                return JsonSerializer.Serialize(false);
        ...
        private string Request_GenerateToken(NVS.HttpRequestDetails IncomeData)
        {
            if (IncomeData.UrlList.Length > 2)
            {
                if (IncomeData.UrlList[1].ToLower() != "generate")
                {
                    ...
                    {
                        UID = "",
                        Code = NVC.ErrorNoList.UnknownError,
                ...
                if (IncomeData.PostParams.ContainsKey("data") == false)
                {
                    ...
                    {
                        UID = "",
                        Code = NVC.ErrorNoList.MissingArgument,
                ...
                try
                {
                    ...
                    if (Notus.Wallet.Block.Exist(NVG.Settings.Network, NVG.Settings.Layer, tmpTokenObj.Info.Tag) == true)
                    {
                        ...
                        {
                            UID = "",
                            Code = NVC.ErrorNoList.TagExists,
                            Status = "TagExists"
                        });
                    }
                    ...
                    if (Notus.Wallet.ID.Verify(TokenRawDataForSignText, tmpTokenObj.Creation.Sign, tmpTokenObj.Creation.PublicKey) == false)
                    {
                        ...
                        {
                            UID = "",
                            Code = NVC.ErrorNoList.WrongSign,
                            Status = "WrongSign"
                        });
                    }
                    ...
                    if (string.Equals(WalletKeyStr, tmpOwnerWalletStr) == false)
                    {
                        ...
                        {
                            UID = "",
                            Code = NVC.ErrorNoList.WrongAccount,
                            Status = "WrongAccount"
                        });
                    }
                    ...
                    long tmpFeeVolume = Notus.Wallet.Fee.Calculate(tmpTokenObj, NVG.Settings.Network);
                    ...
                    */
                    ...
                    if (tmpAddResult == true)
                    {
                        return JsonSerializer.Serialize(new NVS.BlockResponseStruct()
                        {
                            ...
                            Code = NVC.ErrorNoList.AddedToQueue,
                            Status = "AddedToQueue"
                        });
                    }
        ...
        private string Request_ApproveMultiTransaction(NVS.HttpRequestDetails IncomeData)
        {
            ...
            */
            ...
            ObjMp_MultiSignPool.Each((tmpMultiKeyId, multiTransferList) =>
            {
            ...
            long transferFee = Notus.Wallet.Fee.Calculate(
            ...
            ulong validatorUnlockTime = ND.ToLong(
                ND.ToDateTime(uidList[txTime].Sender.CurrentTime).AddDays(30)
            );
        ...
        private string Request_Balance(NVS.HttpRequestDetails IncomeData)
        {
            ...
            if (PrettyCheckForRaw(IncomeData, 2) == true)
            {
                return JsonSerializer.Serialize(balanceResult, NVC.JsonSetting);
            }
        ...
        private string Request_NFTImageList(NVS.HttpRequestDetails IncomeData)
        {
            ...
            string tmpListingDir = IO.GetFolderName(
                NVG.Settings.Network,
                NVG.Settings.Layer,
                NVC.StorageFolderName.Storage
            ) + tmpWalletKey + Path.DirectorySeparatorChar;
            IO.CreateDirectory(tmpListingDir);
        ...
        private string Request_Metrics(NVS.HttpRequestDetails IncomeData)
        {
            ...
            if (IncomeData.UrlList.Length > 1)
            {
                ...
                {
                    ulong tmpTotalBlock = (ulong)GiveMeList(NVE.NetworkNodeType.All).Count;
                    if (NVG.Settings.PrettyJson == true)
                    {
                ...
                {
                    ulong tmpTotalBlock = (ulong)GiveMeList(NVE.NetworkNodeType.Master).Count;
                    if (NVG.Settings.PrettyJson == true)
                    {
                        return JsonSerializer.Serialize(new NVS.MetricsResponseStruct()
                        {
                ...
                {
                    ulong tmpTotalBlock = (ulong)GiveMeList(NVE.NetworkNodeType.Main).Count;
                    if (NVG.Settings.PrettyJson == true)
                    {
                        return JsonSerializer.Serialize(new NVS.MetricsResponseStruct()
                        {
                ...
                {
                    ulong tmpTotalBlock = (ulong)GiveMeList(NVE.NetworkNodeType.Replicant).Count;
                    if (NVG.Settings.PrettyJson == true)
                    {
                        return JsonSerializer.Serialize(new NVS.MetricsResponseStruct()
                        {
                ...
                {
                    ulong tmpTotalBlock = (ulong)NVG.Settings.LastBlock.info.rowNo;
                    if (NVG.Settings.PrettyJson == true)
                    {
                        return JsonSerializer.Serialize(new NVS.MetricsResponseStruct()
                        {
        ...
        private string Request_Online(NVS.HttpRequestDetails IncomeData)
        {
            ...
            {
                return JsonSerializer.Serialize(IncomeData, NVC.JsonSetting);
            }
